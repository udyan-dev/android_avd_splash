...
List<Curve> trimCurves(List<Curve> curves, double start, double end, double offset) {
  ...
  for (final curve in curves) {
    ...
    for (var s = 0; s < cubics; s++) {
      ...
      final from = walked / total, to = length <= 0 ? walked / total : (walked + length) / total;
      walked += length;
      ...
      if (best <= 0 || to - from <= 0) {
        // Outside the window: collapse onto the end nearest to it.
        final at = to <= windows.first[0] ? segment[3] : segment[0];
        piece = [at, at, at, at];
      } else {
